  ...
  repository: {
    ...
    employees: {
      ...
      findByStaffId: async (staffId: string): Promise<RepositoryResponse> => {
        const invokeFn = await getInvoke();
        if (!invokeFn) throw new Error('Tauri API not available');
        return await invokeFn<RepositoryResponse>('repo_employees_find_by_staff_id', { staff_id: staffId });
      },
    },
    ...
    leaveBalances: {
      findByStaffId: async (staffId: string): Promise<RepositoryResponse> => {
        const invokeFn = await getInvoke();
        if (!invokeFn) throw new Error('Tauri API not available');
        return await invokeFn<RepositoryResponse>('repo_leave_balances_find_by_staff_id', { staff_id: staffId });
      },
    },
