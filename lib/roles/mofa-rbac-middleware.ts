...
import type { AuthUser } from '../auth/auth-proxy'
import { canApproveLeave } from '../compliance-utils'
import {
  isAdminRole,
  isHRRole,
  isEmployeeRole,
...
export async function canViewLeaveRequest(
  ...
  try {
    ...
    // HR roles, Chief Director, and System Admin: Can view all
    ...
    if (
      ...
      isUnitHeadRole(role) ||
      isDirectorRole(role) ||
      // Legacy role checks (these get normalized by mapToMoFARole, but check explicitly for safety)
